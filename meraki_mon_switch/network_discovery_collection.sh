...
#What model is the Switch?
  ...
  MODEL9200L=$(cat /var/lib/tftpboot/mon_switch/nwd-${IP}-shver | grep "Model Number" | grep C9200L | tr -cd 'C9200L' | sed 's/.$//')
...
#What model is the Switch?
  ...
  MODEL9300L=$(cat /var/lib/tftpboot/mon_switch/nwd-${IP}-shver | grep "Model Number" | grep C9300L | tr -cd 'C9300L')
...
#What model is the Switch?
  MODEL9500=$(cat /var/lib/tftpboot/mon_switch/nwd-${IP}-shver | grep "cisco C9500" | cut -c7- | sed 's/-.*//')
