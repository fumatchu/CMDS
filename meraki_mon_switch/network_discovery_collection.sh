...
#What model is the Switch?
  MODEL9300=$(cat /var/lib/tftpboot/mon_switch/nwd-${IP}-shver | grep "Model Number" | grep C9300 | tr -cd 'C9300' | sed 's/^\(.....\).*/\1/')
  MODEL9300L=$(cat /var/lib/tftpboot/mon_switch/nwd-${IP}-shver | grep "Model Number" | grep C9300L | tr -cd 'C9300L' | sed 's/^\(......\).*/\1/')
...
#What model is the Switch?
  MODEL9500=$(cat /var/lib/tftpboot/mon_switch/nwd-${IP}-shver | grep "cisco C9500" | cut -c7- | sed 's/-.*//' | sed 's/^\(.....\).*/\1/')
