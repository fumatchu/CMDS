...
echo "Actual Provisioned Switches" | tee -a /root/.meraki_mon_switch/logs/network_discovery > /dev/null 2>&1
...
#Is the Switch already provisioned?
    MUSER=$(cat /var/lib/tftpboot/mon_switch/nwd-${IP}-shrunn | grep -Eo -m 1 "username meraki-user" | cut -c10-)
if [ "$MUSER" = "meraki-user" ]; then
    ...
    sed -i "0,/${IP}/d" /root/.meraki_mon_switch/ip_list
    else
    echo " "
...
cat /root/.meraki_mon_switch/ip_list >> /root/.meraki_mon_switch/logs/network_discovery
